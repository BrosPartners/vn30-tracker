...
def screen_profit(stock: StockInput) -> ScreenResult:
    """Dieu 4.3.1.d: LNST khong am, chi xet BCTC co y kien chap nhan toan phan."""
    ...
    if stock.audit_opinion != "unqualified":
        return ScreenResult(stock.symbol, "profit", ref, False,
                            "Chưa xác nhận ý kiến kiểm toán chấp nhận toàn phần")
...
def test_json_co_du_cac_khoi_web_can():
    ...
    dong = kq["stocks"][0]
    assert set(dong) >= {"symbol", "gtvh_rank", "gtvh_ty", "gtgd_kl_ty",
                         "klgd_kl", "turnover", "free_float", "ket_luan", "screens"}
...
def test_app_js_chi_dung_truong_co_that_trong_json():
    ...
    hop_le = {
        ...
        "in_previous_basket", "ket_luan", "screens", "step", "rule_ref", "passed",
        "message", "shortfall",
    }
...
function the(s, canhBao) {
  ...
  el.innerHTML = `<b>${s.symbol}</b> — hạng ${s.gtvh_rank} vốn hóa,
                  GTVH ${soTy(s.gtvh_ty)} tỷ<ul>${ly_do}</ul>`;
  return el;
}
